...
/* (indirect,X): zp = (operand + X) & 0xFF, addr = mem[zp] | mem[zp+1]<<8 */
static uint16_t am_idx(cpu6502_t*c){uint8_t zp=(uint8_t)(rd(c,c->pc++)+c->x);return rd(c,zp)|((uint16_t)rd(c,(uint8_t)(zp+1))<<8);}
/* (indirect),Y: addr = mem[zp] | mem[zp+1]<<8; Y прибавляется вызывающим кодом */
...
uint32_t cpu6502_step(cpu6502_t*c){
    ...
    case 0x01: a=am_idx(c);                               c->a|=rd(c,a);                     NZ(c,c->a); break;
    ...
    case 0x11: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); c->a|=rd(c,a); NZ(c,c->a); break;
    case 0x13: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); slo(c,a); break;
    ...
    case 0x19: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->a|=rd(c,a); NZ(c,c->a); break;
    case 0x1A: break;
    case 0x1B: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); slo(c,a); break;
    case 0x1C: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rd(c,a); break;
    case 0x1D: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); c->a|=rd(c,a); NZ(c,c->a); break;
    case 0x1E: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); asl_m(c,a); break;
    case 0x1F: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); slo(c,a); break;
    case 0x20: a=r16(c,c->pc); c->pc+=2; push(c,(uint8_t)((c->pc-1)>>8)); push(c,(uint8_t)(c->pc-1)); c->pc=a; break;
    case 0x21: a=am_idx(c);                               c->a&=rd(c,a); NZ(c,c->a); break;
    ...
    case 0x31: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); c->a&=rd(c,a); NZ(c,c->a); break;
    case 0x33: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); rla(c,a); break;
    ...
    case 0x39: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->a&=rd(c,a); NZ(c,c->a); break;
    case 0x3A: break;
    case 0x3B: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); rla(c,a); break;
    case 0x3C: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rd(c,a); break;
    case 0x3D: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); c->a&=rd(c,a); NZ(c,c->a); break;
    case 0x3E: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rol_m(c,a); break;
    case 0x3F: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rla(c,a); break;
    case 0x40: c->flags=pop(c)|FLAG_B; c->pc=pop(c)|((uint16_t)pop(c)<<8); break;
    case 0x41: a=am_idx(c);                               c->a^=rd(c,a); NZ(c,c->a); break;
    ...
    case 0x51: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); c->a^=rd(c,a); NZ(c,c->a); break;
    case 0x53: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); sre(c,a); break;
    ...
    case 0x59: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->a^=rd(c,a); NZ(c,c->a); break;
    case 0x5A: break;
    case 0x5B: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); sre(c,a); break;
    case 0x5C: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rd(c,a); break;
    case 0x5D: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); c->a^=rd(c,a); NZ(c,c->a); break;
    case 0x5E: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); lsr_m(c,a); break;
    case 0x5F: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); sre(c,a); break;
    case 0x60: c->pc=pop(c)|((uint16_t)pop(c)<<8); c->pc++; break;
    case 0x61: a=am_idx(c);                               adc(c,rd(c,a));                     break;
    ...
    case 0x71: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); adc(c,rd(c,a)); break;
    case 0x73: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); rra(c,a); break;
    ...
    case 0x79: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); adc(c,rd(c,a)); break;
    case 0x7A: break;
    case 0x7B: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); rra(c,a); break;
    case 0x7C: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rd(c,a); break;
    case 0x7D: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); adc(c,rd(c,a)); break;
    case 0x7E: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); ror_m(c,a); break;
    case 0x7F: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rra(c,a); break;
    case 0x80: rd(c,c->pc++); break;
    case 0x81: a=am_idx(c);                               wr(c,a,c->a);                       break;
    ...
    case 0xA1: a=am_idx(c);                               c->a=rd(c,a); NZ(c,c->a); break;
    ...
    case 0xB1: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); c->a=rd(c,a); NZ(c,c->a); break;
    case 0xB3: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); c->a=c->x=rd(c,a); NZ(c,c->a); break;
    ...
    case 0xB9: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->a=rd(c,a); NZ(c,c->a); break;
    case 0xBA: c->x=c->sp; NZ(c,c->x); break;
    case 0xBB: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->sp=c->sp&rd(c,a); c->x=c->sp; c->a=c->sp; break;
    case 0xBC: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); c->y=rd(c,a); NZ(c,c->y); break;
    case 0xBD: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); c->a=rd(c,a); NZ(c,c->a); break;
    case 0xBE: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->x=rd(c,a); NZ(c,c->x); break;
    case 0xBF: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); c->a=c->x=rd(c,a); NZ(c,c->a); break;
    case 0xC0: cmp(c,c->y,rd(c,c->pc++)); break;
    case 0xC1: a=am_idx(c);                               cmp(c,c->a,rd(c,a)); break;
    ...
    case 0xD1: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); cmp(c,c->a,rd(c,a)); break;
    case 0xD3: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); dcp(c,a); break;
    ...
    case 0xD9: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); cmp(c,c->a,rd(c,a)); break;
    case 0xDA: break;
    case 0xDB: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); dcp(c,a); break;
    case 0xDC: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rd(c,a); break;
    case 0xDD: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); cmp(c,c->a,rd(c,a)); break;
    case 0xDE: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); v=rd(c,a)-1; NZ(c,v); wr(c,a,v); break;
    case 0xDF: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); dcp(c,a); break;
    case 0xE0: cmp(c,c->x,rd(c,c->pc++)); break;
    case 0xE1: a=am_idx(c);                               sbc(c,rd(c,a)); break;
    ...
    case 0xF1: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); sbc(c,rd(c,a)); break;
    case 0xF3: a=am_idy(c)+c->y; cy+=(((a^(a-c->y))>>8)&1); isb(c,a); break;
    ...
    case 0xF9: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); sbc(c,rd(c,a)); break;
    case 0xFA: break;
    case 0xFB: a=r16(c,c->pc)+c->y; c->pc+=2; cy+=(((a^(a-c->y))>>8)&1); isb(c,a); break;
    case 0xFC: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); rd(c,a); break;
    case 0xFD: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); sbc(c,rd(c,a)); break;
    case 0xFE: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); v=rd(c,a)+1; NZ(c,v); wr(c,a,v); break;
    case 0xFF: a=r16(c,c->pc)+c->x; c->pc+=2; cy+=(((a^(a-c->x))>>8)&1); isb(c,a); break;
    }
