...
static const uint8_t gamma_pos[15] = {0x0F,0x31,0x2B,0x0C,0x0E,0x08,0x4E,0xF1,0x37,0x07,0x10,0x03,0x0E,0x09,0x00};
static const uint8_t gamma_neg[15] = {0x00,0x0E,0x14,0x03,0x11,0x07,0x31,0xC1,0x48,0x08,0x0F,0x0C,0x31,0x36,0x0F};
...
static void lcd_init_sequence(void) {
    lcd_write_cmd(ILI9341_SWRESET); sleep_ms(150);
    lcd_write_cmd(ILI9341_DISPOFF);
    lcd_write_cmd(ILI9341_PWCTRB); lcd_write_data(0x00); lcd_write_data(0xC1); lcd_write_data(0x30);
    lcd_write_cmd(ILI9341_POWER_SEQ); lcd_write_data(0x64); lcd_write_data(0x03); lcd_write_data(0x12); lcd_write_data(0x81);
    lcd_write_cmd(ILI9341_DTCA); lcd_write_data(0x85); lcd_write_data(0x00); lcd_write_data(0x78);
    lcd_write_cmd(ILI9341_PWCTRA); lcd_write_data(0x39); lcd_write_data(0x2C); lcd_write_data(0x00); lcd_write_data(0x34); lcd_write_data(0x02);
    lcd_write_cmd(ILI9341_PRC); lcd_write_data(0x20);
    lcd_write_cmd(ILI9341_DTCB); lcd_write_data(0x00); lcd_write_data(0x00);
    lcd_write_cmd(ILI9341_PWCTR1); lcd_write_data(0x23);
    lcd_write_cmd(ILI9341_PWCTR2); lcd_write_data(0x10);
    lcd_write_cmd(ILI9341_VMCTR1); lcd_write_data(0x3E); lcd_write_data(0x28);
    lcd_write_cmd(ILI9341_VMCTR2); lcd_write_data(0x86);
    lcd_write_cmd(ILI9341_MADCTL); lcd_write_data(LCD_MADCTL);
    lcd_write_cmd(ILI9341_PIXFMT); lcd_write_data(0x55);
    lcd_write_cmd(ILI9341_FRMCTR1); lcd_write_data(0x00); lcd_write_data(0x18);
    lcd_write_cmd(ILI9341_DFUNCTR); lcd_write_data(0x08); lcd_write_data(0x82); lcd_write_data(0x27);
    lcd_write_cmd(ILI9341_3GAMMA_EN); lcd_write_data(0x00);
    lcd_write_cmd(ILI9341_GAMMASET); lcd_write_data(0x01);
    lcd_write_cmd(ILI9341_GMCTRP1); for (int i = 0; i < 15; i++) lcd_write_data(gamma_pos[i]);
    lcd_write_cmd(ILI9341_GMCTRP2); for (int i = 0; i < 15; i++) lcd_write_data(gamma_neg[i]);
    lcd_write_cmd(ILI9341_SLPOUT); sleep_ms(120);
    lcd_write_cmd(ILI9341_DISPON); sleep_ms(25);
}

void display_init(void) {
    for (int i = 0; i < 8; i++) { gpio_init(LCD_DATA_BASE + i); gpio_set_dir(LCD_DATA_BASE + i, GPIO_OUT); }
    ...
    /* BL (GP11) подтянут к 3V3 */
    lcd_hard_reset();
    lcd_init_sequence();
}

/* Fill entire screen with one color via streaming */
void display_fill(uint16_t color) {
    ...
    for (uint32_t i = 0; i < LCD_WIDTH * LCD_HEIGHT; i++) {
        lcd_bus_write(hi); lcd_wr_strobe();
        lcd_bus_write(lo); lcd_wr_strobe();
    }
}

/* Fill rectangle via streaming */
...
void display_set_pixel(int x, int y, uint16_t color) {
    ...
    lcd_write_cmd(ILI9341_RAMWR);
    gpio_put(LCD_RS_PIN, 1);
    lcd_bus_write((uint8_t)(color >> 8)); lcd_wr_strobe();
    lcd_bus_write((uint8_t)(color & 0xFF)); lcd_wr_strobe();
}

void display_blit(const uint8_t *src, const uint16_t *lut, int x, int y, int w, int h) {
    display_stream_begin(x, y, w, h);
    display_stream_pixels(src, lut, w, h);
    display_stream_end();
}
