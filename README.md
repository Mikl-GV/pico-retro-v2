...
- Дисплей **WF28ETLAJDNN0** — 2.8", 240×320, контроллер ILI9341V, **8-бит 8080, вывод через PIO + DMA**
- Джойстик **Sega Mega Drive 2** (DB9), 3/6-button
- **microSD** (SPI, bit-bang) — ROM/данные
- **Композитный видеовыход** (PAL/NTSC) через pico-extras `pico_scanvideo`
- **Звук** через PWM
...
| Модуль | Статус |
|---|---|
| Дисплей ILI9341V (8-бит 8080) | Готов: команды bit-bang, пиксели PIO+DMA |
| Джойстик MD2 (3/6-button) | Готов |
| microSD (SPI bit-bang, чтение/запись блоков) | Готов (драйвер блоков) |
| Звук (PWM) | Заглушка (beep), нужен микшер |
| Композитный видео | Каркас на pico_scanvideo, TODO рендер кадра |
...
| Сигнал | Pico GPIO |
|---|---|
| Композитный видео | 12 |
| Звук L / R (PWM, через RC-фильтр) | 13 / 14 |
